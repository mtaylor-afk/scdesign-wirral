...
/*
 * Entries flagged `"inline": true` are ALSO emitted as base64 data URIs in
 * src/lib/brand-assets.ts. The OG card (app/opengraph-image.tsx) renders through
 * next/og at build time and needs the artwork as bytes; reading it off disk
 * there means resolving a path against process.cwd(), which is whatever
 * directory the build (or a dev server) happened to start in. Baking the bytes
 * into a module removes the filesystem from the render path entirely.
 */
const inlined = results.filter((r) => r.inline);
if (inlined.length) {
  const entries = inlined.map((r) => {
    const buf = fs.readFileSync(path.join(root, "public", r.out));
    const mime = r.out.endsWith(".png") ? "image/png" : "image/jpeg";
    return `  ${r.id}: {\n    src: "data:${mime};base64,${buf.toString("base64")}",\n    width: ${r.width},\n    height: ${r.height},\n    alt: ${JSON.stringify(r.alt)},\n  },`;
  });
  fs.writeFileSync(
    path.join(root, "src/lib/brand-assets.ts"),
    `/**\n * GENERATED by scripts/process-brief-images.mjs — do not edit by hand.\n *\n * Brand artwork as base64 data URIs, for next/og cards that are rendered at\n * build time and cannot rely on process.cwd() to find public/.\n */\nexport const brandAssets = {\n${entries.join("\n")}\n} as const;\n`
  );
  console.log(`Wrote src/lib/brand-assets.ts (${inlined.length} inlined)`);
}
